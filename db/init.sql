...
CREATE TABLE IF NOT EXISTS `stripe_topups` (
    `ID_STRIPE_TOPUP` INT UNSIGNED NOT NULL AUTO_INCREMENT,
    `ID_USER` INT UNSIGNED NOT NULL,
    `ID_CUSTOMER` INT UNSIGNED NOT NULL,
    `AMOUNT` DECIMAL(10,2) NOT NULL,
    ...
    PRIMARY KEY (`ID_STRIPE_TOPUP`),
    UNIQUE KEY `UK_STRIPE_SESSION` (`CHECKOUT_SESSION_ID`),
    KEY `IDX_STRIPE_USER` (`ID_USER`),
    KEY `IDX_STRIPE_CUSTOMER` (`ID_CUSTOMER`),
    CONSTRAINT `FK_STRIPE_TOPUP_USER`
        FOREIGN KEY (`ID_USER`) REFERENCES `users`(`ID_USER`)
        ON DELETE CASCADE,
    CONSTRAINT `FK_STRIPE_TOPUP_CUSTOMER`
        FOREIGN KEY (`ID_CUSTOMER`) REFERENCES `customers`(`ID_CUSTOMER`)
        ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
